...
def test_run_all_matched(runner, vault_file):
    ...
    result = runner.invoke(
        env_check_group, ["run", "--vault", vault_file, "--password", PASSWORD], env=env
    )
...
def test_run_strict_exits_nonzero_on_issue(runner, vault_file):
    ...
    result = runner.invoke(
        env_check_group,
        ["run", "--vault", vault_file, "--password", PASSWORD, "--strict"],
        env=env,
    )
    assert result.exit_code != 0
...
def test_show_extra_flag(runner, vault_file):
    ...
    result = runner.invoke(
        env_check_group,
        ["run", "--vault", vault_file, "--password", PASSWORD, "--show-extra"],
        env=env,
    )
